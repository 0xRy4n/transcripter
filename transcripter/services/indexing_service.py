...
class IndexingService:
    ...
    def index_all(self) -> List[Dict[str, Union[List[str], Dict[str, List[str]]]]]:
        ...
        results.extend(self._index_entities(self.config.playlists, self._index_playlist, "playlists"))
        results.extend(self._index_entities(self.config.channels, self._index_channel, "channels"))
        results.extend(self._index_entities(self.config.videos, self._index_video, "videos"))

        return results

    def _index_entities(self, entities: Optional[List[str]], index_method: Any, entity_type: str) -> List[Dict[str, Union[List[str], Dict[str, List[str]]]]]:
        """
    ...
    def _index_playlist(self, playlist_id: str) -> Dict[str, Union[List[str], Dict[str, List[str]]]]:
        """
        ...
        video_details: Dict[str, Dict[str, Union[str, int]]] = self.youtube_manager.get_all_video_details_from_playlist(playlist_id)
        logger.info(f"Retrieved {len(video_details)} videos from playlist {playlist_id}")
        return self._index_videos(video_details)

    def _index_channel(self, channel_id: str) -> Dict[str, Union[List[str], Dict[str, List[str]]]]:
        """
        ...
        video_details: Dict[str, Dict[str, Union[str, int]]] = self.youtube_manager.get_all_video_details_from_channel(channel_id)
        return self._index_videos(video_details)

    def _index_video(self, video_id: str) -> Dict[str, Union[List[str], Dict[str, List[str]]]]:
        """
        ...
        video_details: Dict[str, Union[str, int]] = self.youtube_manager.get_video_details(video_id)
        return self._index_videos({video_id: video_details})

    def _index_videos(self, video_details: Dict[str, Dict[str, Union[str, int]]]) -> Dict[str, Union[List[str], Dict[str, List[str]]]]:
        """
        ...
        for video_id, details in video_details.items():
            try:
                ...
                if not self.redis_manager.document_exists(video_id):
                    transcript: Optional[List[Dict[str, Union[str, float]]]] = self.youtube_manager.get_transcript_details_from_video(video_id)
                    if transcript:
        ...
        logger.info(f"Indexing complete. Total indexed: {len(indexed)}, Newly indexed: {len(newly_indexed)}")
    ...
    def _process_transcript(self, video_id: str, details: Dict[str, Union[str, int]], transcript: List[Dict[str, Union[str, float]]]) -> None:
        """
        ...
        grouped_transcript: List[List[Dict[str, Union[str, float]]]] = list(zip_longest(*[iter(transcript)] * chunk_size, fillvalue=None))

        for i, group in enumerate(grouped_transcript):
            group = [chunk for chunk in group if chunk is not None]  # Remove None values
